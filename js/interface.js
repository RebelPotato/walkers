...
interfaceSetup = function() {
  ...
  var lazer_speed_sel = document.getElementById("lazer_speed");
  for(var k = 0; k < lazer_speed_sel.options.length; k++) {
    if(lazer_speed_sel.options[k].value == config.lazer_speed) {
      lazer_speed_sel.options[k].selected = true;
      break;
    }
  }
  lazer_speed_sel.onchange = function() {
    config.lazer_speed = parseFloat(lazer_speed_sel.value);
  }
}